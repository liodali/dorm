...
''';
  ...
  /// References schemas from entity .schema.g.dart files via extensions
  String _generateDatabaseSchema(
    String dbClassName,
    List<_EntityInfo> entities,
  ) {
    // Reference schemas from entity extensions (generated by SchemaGenerator)
    final schemaRefs = entities
        .map((e) => '${e.className}Schema.schema')
        .join(', ');

    return '''// Database schemas for $dbClassName
// Schemas are generated per-entity in .schema.g.dart files
// Access via: EntityName.schema or EntityNameSchema.schema

// All schemas for this database (references entity schemas)
final ${_toCamelCase(dbClassName)}Schemas = <DatabaseSchema>[
  $schemaRefs,
];
''';
  }
...
extension ${dbClassName}Lifecycle on $dbClassName {
  ...
  Future<void> createSchema() async {
    ...
    for (final schema in $junctionSchemaListVar) {
      await schemaManager.createTable(schema);
    }
  }
  ...
  Future<void> dropSchema() async {
    ...
    for (final schema in $junctionSchemaListVar.reversed) {
      await schemaManager.dropTable(schema);
    }
    
    // Drop entity tables
    for (final schema in $schemaListVar.reversed) {
      await schemaManager.dropTable(schema);
    }
  }
